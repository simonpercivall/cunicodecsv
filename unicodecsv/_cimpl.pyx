...
from cpython cimport PyUnicode_FromEncodedObject
...
class UnicodeReader(object):
    ...
    def next(self):
        ...
        cdef char * encoding = self.encoding
        cdef char * encoding_errors = self.encoding_errors
        return [value if isinstance(value, float) else
                PyUnicode_FromEncodedObject(value, encoding, encoding_errors) for value in row]
